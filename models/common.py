...
class autoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        #   opencv:     imgs = cv2.imread('images.jpg')[:,:,::-1]  # HWC BGR to RGB x(720,1280,3)
        #   PIL:        imgs = Image.open('images.jpg')  # HWC x(720,1280,3)
        ...
        if not isinstance(imgs, list):
            imgs = [imgs]
        shape0, shape1 = [], []  # images and inference shapes
        batch = range(len(imgs))  # batch size
        for i in batch:
            imgs[i] = np.array(imgs[i])  # to numpy
            if imgs[i].shape[0] < 5:  # images in CHW
